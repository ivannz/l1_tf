...
cdef extern from "l1_tf/l1tf.c":
    ...
    int c_l1tf "l1tf" (const int n, const double *y, const double C, double *x) nogil
    double c_l1tf_Cmax "l1tf_lambdamax"(const int n, double *y) nogil
...
def l1tf_Cmax(double[::1] X):
    ...
    cdef double C_max = c_l1tf_Cmax(X.shape[0], &X[0])
    if C_max < 0:
        raise RuntimeError("""Cannot solve for adaptive regularization parameter.""")
    return C_max
